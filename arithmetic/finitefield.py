import random
from arithmetic.polynomial import polynomialsOver
...
def shamir_random_polynomial(degree, secret, modulus):
   """ Generates a random polynomial with positive coefficients.
   """
   if degree < 0:
      raise ValueError('Degree must be a non-negative number.')
   ...
   #logging.info("shamir_random_polynomial: {}".format(randomMonicPolynomial))
   return randomMonicPolynomial


def get_polynomial_points(polynomial, num_points):
   """ Calculates the first n polynomial points.
       [ (1, f(1)), (2, f(2)), ... (n, f(n)) ]
   """
   #print("pol: {}".format(polynomial))
   coefficients = polynomial.get_coeff();
   points = []
   for x in range(1, num_points + 1):
      ...
      for i in range(1, len(coefficients)):
         exponentiation = (x ** i)
         term = (coefficients[i] * exponentiation)
         y = (y + term)
